...
class ClassicPrisonPlayer extends CorePlayer {
	...
	public function attack($damage, EntityDamageEvent $source) {
		if($source instanceof EntityDamageByEntityEvent and ($attacker = $source->getDamager()) instanceof ClassicPrisonPlayer) {
			if($attacker->getState() === CorePlayer::STATE_PLAYING) {
				if($this->lastPlayerDamager instanceof ClassicPrisonPlayer and $this->lastPlayerDamager !== $attacker) {
		...
		if($source->getFinalDamage() >= $this->getHealth()) {
			switch($source->getCause()) {
				default:
					...
					if($attacker instanceof ClassicPrisonPlayer and floor(microtime(true) - $this->lastPlayerDamageTime) <= 12) {
				...
				case EntityDamageEvent::CAUSE_ENTITY_ATTACK:
					if($source instanceof EntityDamageByEntityEvent) {
						...
						if($attacker instanceof ClassicPrisonPlayer) {
							...
							if($attacker->shootingEntity instanceof ClassicPrisonPlayer) {
